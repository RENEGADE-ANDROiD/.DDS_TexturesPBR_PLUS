...
| Path | Role |
|------|------|
| `zscript.zc` | ZScript entry point (`version "4.10.0"`); `#include` chain + inline EventHandlers/classes (smoke FX, rain, map probes) |
...
| `terrain.txt` | Splash actor assignments (points at `DDS_NJ*` actors in `DECORATE.Splash`) |
| `ANIMDEFS.txt` / `ANIMDEFS2` | OTEX-style texture animation sequences |
| `cvarinfo` | User CVars (gore amount, teleporter flats, auto-clear gore, fog, rain/weather) |
...
| Gore ZScript | `zscript/Gore/*.zs`, `DropletsEventHandler` | Yes (EXTRAS branch) |
...
| `DDS_64*` | Doom 64-style decoration variants (e.g. `DDS_64EvilEye`, `DDS_64Column`) |
